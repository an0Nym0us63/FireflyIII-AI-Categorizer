...
const groundingInstruction = `WEB SEARCH: You have Google Search available and should USE IT whenever the merchant is not a large, unambiguous national brand you are certain about. In particular, search when the label looks like a LOCAL or independent business — typically a name plus a town (e.g. "LE TONNEAU COMBRONDE", "SCP DE VETERINA MOZA", "NEW COFAST"), an obscure or abbreviated company name, or a bank-terminal string. Do NOT guess the activity of a local business from its name alone (a name like "Le Tonneau" is NOT necessarily a bar/restaurant); look it up. Search the web for the merchant label (name + town) taken from the description, find the ACTUAL business and its real activity, then classify from that. Only skip the search for clearly recognizable national chains (e.g. Carrefour, Amazon, SNCF, Netflix). Never search for anything other than identifying this merchant. Whatever you do, still return ONLY the required JSON object.`
