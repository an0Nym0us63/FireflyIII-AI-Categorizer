...
func buildUserPrompt(req Request) string {
	...
	if req.TagSuggestion {
		...
		if strings.TrimSpace(req.ExtraContext) != "" {
			fmt.Fprintf(&sb, "- The exact contents of this purchase are given below. Be thorough and specific: derive SEVERAL tags (use the full budget of %d when the product supports it) describing the product from different angles — its BRAND, its PRODUCT TYPE/object, and a clear ATTRIBUTE or use. Example: contents \"Anker Soundcore casque bluetooth\" → anker, casque audio, bluetooth (three useful tags, not just \"anker\"). When SEVERAL distinct products are listed, give one CONCRETE product-type tag per item (e.g. contents \"gourde ... ciseaux ...\" → gourde, ciseaux) rather than a single vague umbrella theme like \"scolarité\". Adapt to whatever the products actually are. Facts read directly from the contents (brand, product type) are certain — mark those CLASSIFIED.\n", max)
		}
